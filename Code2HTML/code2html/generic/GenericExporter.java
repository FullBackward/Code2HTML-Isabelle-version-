...
public abstract class GenericExporter {
    ...
    // Add set to track used styles
    protected Set<Integer> usedStyles = new HashSet<Integer>();
    ...
    // Add method to get used styles
    public Set<Integer> getUsedStyles() {
        return usedStyles;
    }
    ...
    public String getStyleString( ) {
        // Clear used styles and collect them first
        usedStyles.clear();
        collectUsedStyles();
    ...
    // Add method to collect used styles by scanning content first
    private void collectUsedStyles() {
        int physicalFirst = 0;
        int physicalLast = buffer.getLineCount() - 1;

        if ( this.selection == null ) {
            this.scanStyles( physicalFirst, physicalLast );
        }
        else {
            int last = 0;
            ...
            for ( int i = 0; i < selection.length; i++ ) {
                ...
                this.scanStyles( Math.max( physicalFirst, lastLine + 1 ) , physicalLast );
    ...
    // Add method to scan and collect used styles without generating output
    protected void scanStyles( int first, int last ) {
        ...
        for ( int i = first; i <= last; i++ ) {
            ...
            // Scan tokens to collect used styles
            for (;;) {
                byte id = tokens.id;
                if (id == Token.END) {
                    break;
