...
public abstract class GenericDocument {
    ...
    // Add reference to exporter to get used styles
    ...
    // Add method to set exporter reference
    public void setExporter(GenericExporter exporter) {
    ...
    public void openStyle( Writer out ) throws IOException {
        ...
        // Only generate styles for actually used style IDs
        if (exporter != null) {
            Set<Integer> usedStyles = exporter.getUsedStyles();
            for ( Integer styleId : usedStyles ) {
                out.write( this.style.style( styleId, this.syntaxStyles[styleId] ) );
            }
        } else {
            // Fallback to original behavior if exporter is not set
